...
function addButton(TimeOut = 3000) {
    if (TimeOut <= 0) { return; }
    $('iframe').each(function(){ 
        var iframe = $(this).contents();
        if ( iframe.find('head').find('style[data-style="legis"]').length == 0 ) {
            iframe.find('head').append("<style type='text/css' data-style='legis'> "
    ...
    setTimeout(function(){ 
        if ( $('div[id^=cke_txaEditor_]').length ) {
            if ( !$('.getLegisButtom').length ) {
                $('div[id^=cke_txaEditor_]').each(function(index){ 
                    $(this).find('span.cke_toolbox').append(htmlButton);
                });
                $('.getLegisButtom').click(function() { initLegis() });
                console.log('addButton Load',TimeOut);
            }
        } else {
        ...
        if ( textSearch.indexOf('#') !== -1) {
            ...
            $.each(arrayTag, function (index, value) {
                ...
                if ( iframe.find('a[name='+refTag+']').length ) {
                    ...
                    var textRef = getNameRef(anchor, iframe, null);
...
function updateRefsTags(iframe) {
    iframe.find('.legis.ref').each(function(){
        ...
        var textRef = getNameRef(anchor, iframe, this_);
